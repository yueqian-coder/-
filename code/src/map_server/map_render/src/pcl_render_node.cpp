...
int _GRID_X_SIZE, _GRID_Y_SIZE, _GRID_Z_SIZE;
...
{
    Eigen::Vector3i idx;
    idx(0) = std::min(std::max(int((pt(0) - _gl_xl) * _inv_resolution), 0), _GRID_X_SIZE - 1);
    idx(1) = std::min(std::max(int((pt(1) - _gl_yl) * _inv_resolution), 0), _GRID_Y_SIZE - 1);
    idx(2) = std::min(std::max(int((pt(2) - _gl_zl) * _inv_resolution), 0), _GRID_Z_SIZE - 1);
...
int main(int argc, char **argv)
{
    ...
    _GRID_X_SIZE = (int)(_x_size * _inv_resolution);
    _GRID_Y_SIZE = (int)(_y_size * _inv_resolution);
    _GRID_Z_SIZE = (int)(_z_size * _inv_resolution);
