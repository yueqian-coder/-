...
static Control
getControl(const QuadrotorSimulator::Quadrotor& quad, const Command& cmd)  //据四旋翼飞行器的当前状态和接收到的控制指令，计算出应该设置给四旋翼飞行器四个电机的转速值  
{
...
static void
cmd_callback(const quadrotor_msgs::SO3Command::ConstPtr& cmd)   //控制指令回调函数
{
...
main(int argc, char** argv)
{
  ...
  ros::Subscriber f_sub =
    n.subscribe("force_disturbance", 100, &force_disturbance_callback,
                ros::TransportHints().tcpNoDelay());    //外部干扰力回调函数
  ros::Subscriber m_sub =
    n.subscribe("moment_disturbance", 100, &moment_disturbance_callback,
                ros::TransportHints().tcpNoDelay());  //外部干扰力矩回调函数
  ...
  const double dt = 1 / simulation_rate;   //单位时间
