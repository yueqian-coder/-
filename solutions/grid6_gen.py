...
list_path = os.path.join(base, 'grid6_list.txt')
...
vel_vals = [1.25, 1.35]
acc_vals = [1.15, 1.25]
kg_vals = [0.88, 0.84, 0.80]

names = []
for vel_s, acc_s, t_global in itertools.product(vel_vals, acc_vals, kg_vals):
    name = f"variant_G6_v{int(vel_s*100):03d}_a{int(acc_s*100):03d}_g{int(t_global*100):02d}"
    var_dir = os.path.join(base, name)
    os.makedirs(var_dir, exist_ok=True)
    content = (
        f"# grid6 aggressive time scales\n"
